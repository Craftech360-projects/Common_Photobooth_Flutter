...
// REMOVED: The AppFlowProvider is no longer needed for this logic.
...
class AuthProvider extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    try {
      ...
      await _updateWorkflowSelection(); // CHANGED: Renamed for clarity
  ...
  // This online verification method now also leads to workflow selection
  ...
  Future<void> logout() async {
    ...
    try {
      ...
      // ADDED: Clear the selected workflow on logout
  ...
  Future<void> _updateWorkflowSelection() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final serviceId = prefs.getString('authenticated_service_id');

      if (serviceId == null) {
        debugPrint('No serviceId found, cannot select a workflow.');
        return;
      }

      final workflowFileName = _getWorkflowNameForServiceId(serviceId);

      if (workflowFileName != null) {
        await prefs.setString('selected_workflow', workflowFileName);
        debugPrint(
            'Selected workflow for serviceId $serviceId: $workflowFileName');
      } else {
        debugPrint(
            'Warning: No workflow mapping found for serviceId: $serviceId');
        // Handle cases where the serviceId does not map to a workflow
        // You could clear it, or set a default.
        await prefs.remove('selected_workflow');
      }
    } on Exception catch (e) {
      debugPrint('Error updating workflow selection: $e');
    }
  }

  String? _getWorkflowNameForServiceId(String serviceId) {
    switch (serviceId) {
      case 'LjCIQ5ONsqCHIHd6Rmyu':
        return 'swaplab.json';
      case 'LxdfSxqisSz6upaGX2Ly':
        return 'packaging.json';
      case 's8I5m3JkBICqt2X1itzQ':
        return 'pixar.json';
      case 'ufD3VgOLuZD14zk601in':
        return 'ghibli.json';
      default:
        // Return null or a default workflow if needed
        return null;
    }
  }
}
