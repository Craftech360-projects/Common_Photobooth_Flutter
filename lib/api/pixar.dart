...
class PixarWorkflow {
  ...
  PixarWorkflow(this._workflow);

  /// Load the workflow from the JSON file
  static Future<PixarWorkflow> getWorkflow() async {
    final jsonString = await rootBundle.loadString('lib/api/pixar.json');
    final workflow = jsonDecode(jsonString);
    return PixarWorkflow(workflow);
  }
